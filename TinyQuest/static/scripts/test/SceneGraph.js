    ...
    game.onload = function() {
      ...
      return this.addEventListener("enterframe", function() {
        ...
        if (currentAnimationFileName) {
          if (target) {
            if (target.getPosition()[0] >= GAME_WIDTH / (3 * root.getScale()[0])) {
              moveVelocityX *= -1;
            }
            if (target.getPosition()[0] <= GAME_HEIGHT / (8 * root.getScale()[0])) {
              moveVelocityX *= -1;
            }
            if (target.getPosition()[1] >= GAME_HEIGHT / (2 * root.getScale()[1])) {
              moveVelocityY *= -1;
            }
            if (target.getPosition()[1] <= GAME_HEIGHT / (8 * root.getScale()[1])) {
              moveVelocityY *= -1;
            }
