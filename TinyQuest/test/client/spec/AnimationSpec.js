(function() {
  ...
  testFrame = function(root, interval, testData, i, j) {
    ...
    return expect(interval.isDone()).toBe(testData[j].isDone[i]);
  };

  describe("Animation", function() {
    return describe("TestInterval", function() {
      ...
      it("AlphaInterval", function() {
        ...
        interval.reset();
        expect(node.getAlpha()).toEqual(0.0);
        return expect(interval.isDone()).toBe(false);
      });
      ...
      it("SourceInterval", function() {
        ...
        expect(sprite.getSrcRect()).toEqual([20, 20, 22, 48]);
        expect(sprite.getCenter()).toEqual([4, 4]);
        return expect(interval.isDone()).toBe(true);
      });
      it("Sequence1", function() {
        ...
        sequence.reset();
        expect(node.getAlpha()).toBe(0.1);
        return expect(sequence.isDone()).toBe(false);
      });
      it("Sequence2", function() {
        ...
        sequence.update();
        expect(node.getAlpha()).toBe(0.1);
        return expect(sequence.isDone()).toBe(true);
      });
      it("Parallel", function() {
        ...
        expect(node.getAlpha()).toBe(0.1);
        expect(node.getRotation()).toBe(0);
        return expect(parallel.isDone()).toBe(false);
      });
      it("Loop count 2", function() {
        ...
        return expect(loop_.isDone()).toBe(true);
      });
