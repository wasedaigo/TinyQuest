	...
	public void SetupBattle() {
		...
		for (int i = 0; i < Constant.GroupCount; i++) {
			for (int j = 0; j < combatUnitGroups[i].combatUnits.Count; j++) {
				...
				combatUnit.remainTime = Mathf.FloorToInt(Random.value * 30);
			}
		}
	}
	
	public void ReduceRemainTime(int groupNo, int index, float delta) {
		CombatUnit combatUnit = this.GetCombatUnit(groupNo, index);
		combatUnit.remainTime -= delta;
	}

	public void ReduceAllRemainTime(float delta) {
		CombatUnitGroup[] combatUnitGroups = this.GetCombatUnits();
		for (int i = 0; i < Constant.GroupCount; i++) {
			for (int j = 0; j < combatUnitGroups[i].combatUnits.Count; j++) {
				CombatUnit combatUnit = combatUnitGroups[i].combatUnits[j];
				combatUnit.remainTime -= delta * combatUnit.userUnit.speed;
			}
		}
	}
}
